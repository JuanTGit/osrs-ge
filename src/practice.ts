...
app.get('/items/search', async (req, res) => {
	...
	const request = await fetch("https://oldschool.runescape.wiki/?title=Module:GEIDs/data.json&action=raw&ctype=application%2Fjson");
	if (!request.ok){
...
const fetchData = async function(): Promise<Price> {
	const request = await fetch("https://prices.runescape.wiki/api/v1/osrs/latest?id=4151");
